...
import com.github.jakub_galazka.java_in_nutshell._5java_io.utils.IndexRecord;
import com.github.jakub_galazka.java_in_nutshell._5java_io.utils.Location;
...
/*
    ...
    * Character data - readable format -> csv, json, xml
    ...
    Files:
    * Serial file / Sequential file - the stream of data that arrives at program or is sent out from it in defined order witch each piece of data following in sequence.
    * Random access file - only applies to file and allows to jump about the file or within the file retrieving or overwriting any data in any location within that file
        -> like database: sort of index showing where particular record is within the file so that it can be read without having to be read through the earlier records first.
*/
...
public class _2JavaIO implements Map<Integer, Location> {

    private static final String LOCATIONS_TXT_PATH = "com/github/jakub_galazka/java_in_nutshell/_5java_io/utils/files/locations.txt";
    private static final String DIRECTIONS_TXT_PATH = "com/github/jakub_galazka/java_in_nutshell/_5java_io/utils/files/directions.txt";
    private static final String LOCATIONS_DAT_PATH = "com/github/jakub_galazka/java_in_nutshell/_5java_io/utils/files/locations.dat"; // .dat -> binary data
    private static final String LOCATIONS_OBJ_PATH = "com/github/jakub_galazka/java_in_nutshell/_5java_io/utils/files/locations_obj.dat";
    private static final String LOCATIONS_RAF_PATH = "com/github/jakub_galazka/java_in_nutshell/_5java_io/utils/files/locations_raf.dat";
    ...
    /*
        ...
        * Buffering prevents reading / writing small amounts of data to the disk, witch is very efficient because the disk drive generally speaking is much slower than computer can process.
          The reason for that is because is that things like the overhead of seeking to the correct track and sector on the disk drive are significant.
          So it actually takes a while to find the correct place on the disk drive to write or read from.
        * Using the buffer data is written / read out in sizable chunks rather than sort of character at a time.
        * Size of buffer can be specified -> DEFAULT: 8k bytes -> 8k bytes put into the buffer in single write / read and be available from there FOR EXAMPLE: (reading) for the Scanner to use it as it needs more data.
    */
    ...
    /*
        ...
        But writing the code this way cause the Exception to be suppressed and the Exception from the Try block is the one that is actually thrown back up the stack.
        So that does actually make sense as it is very likely that whatever problem occurred while opening or writing to the stream is probably of more interest than the error closing the stream.
        In fact the first error is probably the root cause of the error when closing if you think about it.
        Using Try-finally the first error would be hidden by the Exception when closing the stream whereas this method ensures that the first error is the one thrown back.
    */
    private void writingTextDataWithTryWithResources() {
        try (BufferedWriter locFile = new BufferedWriter(new FileWriter(LOCATIONS_TXT_PATH));
             BufferedWriter dirFile = new BufferedWriter(new FileWriter(DIRECTIONS_TXT_PATH))) { // Try-with-resources allows more than one resource to be specified
            // LOGIC START
            for (Location location : locations.values()) {
                locFile.write(location.getLocationID() + "," + location.getDescription() + "\n");
    ...
    private void readingTextDataWithTryCatchUsingScanner() {
        ...
        try {
            ...
            if (scanner != null) {
                scanner.close(); // Scanner close() method also take care of closing any stream that it was using, provided that the stream object implements the Closable interface
            }
        }
    ...
    /*
        ...
        * Advantage of dealing with binary data (byte stream) is that we do not have to pass the data into the various data types that were stored.
        ...
        * BufferInputStream, BufferOutputStream, DataOutputStream, DataOutputStream objects only handle raw bytes.
          So writing or reading either a single byte or a specified number of bytes.
        * Now there are occasions when that is probably all you will need,
          but generally speaking when storing and retrieving data, you probably want to work with the Java types.
        * Java IO package makes this easy by providing the DataInputStream and DataOutputStream classes,
          and they provide methods such as readInt() to read an integer value from the stream.
          And it does this by reading 4 bytes from the stream and building up the integer from those 4 bytes.
    */
    ...
    private void readingBinaryData() {
        try (DataInputStream locFile = new DataInputStream(new BufferedInputStream(new FileInputStream(LOCATIONS_DAT_PATH)))) {
            ...
            while(!eof) {
                ...
                } catch (EOFException e) { // Data stream readers throw an EOFException (end of file exception: subclass of IOException) when there is no more data to be read from the stream
                    eof = true;
                }
            }
            // LOGIC END
    ...
    /*
        ...
        * To write an object to a file, it has to be translated to a format that can be stored to a file and then reassembled into an object later when it is read by an application.
        ...
        * Serializable interface got no methods, it is just used to give JVM, a heads-up that we may want to serialize the object to storage and to read it back again at some point.
        * IMPORTANT: when we make a class serializable it is strongly recommended that we declare a private long field called serialVersionUID
            -> if we do not do so, the compiler will give us a warning.

        serialVersionUID - sort of version number for the class:
        * If we ever change a class, for example, by adding another filed or method or change a field type, we have to change the serialVersionUID value.
        * When the object is read from a stream, the runtime checks the stored serialVersionUID against the one contained within the compiled class file
            -> if they do not match, then there is a compatibility problem and the runtime will throw an InvalidClassException.

        Note that:
        * The ObjectOutputStream implements the DataOutputStream interface and ObjectInputStream implements the DataInputStream.
        * Now the DataOutputStream and DataInputStream interfaces contain the declarations for the writeInt() / readInd() as well as writeUTF() / readUTF() methods and other read / write specific type methods.
        * Because of that, object streams can contain a mix of serialized objects and primitive types -> they are not limited to only containing serialized objects.
    */
    ...
    public void readingObject() {
        try (ObjectInputStream objectsFile = new ObjectInputStream(new BufferedInputStream(new FileInputStream(LOCATIONS_OBJ_PATH)))) {
            ...
            while (!eof) {
                try {
                    Location location = (Location) objectsFile.readObject();
        ...
        } catch (ClassNotFoundException e) { // Thrown when the runtime read an object from the stream, and it cannot find corresponding class on the class path
    ...
    /*
        ...
        * For example if the File Pointer is set to 100, then the next time we call a write / read method the write / read will start at byte position 100 in the file.
        * Now if we are reading an int the byte set positions 100 to 103 would be read since an int is 4 bytes -> IMPORTANT: the byte position is zero based.
        ...
        * Or we could load the entire Index into memory when the application starts -> IDEALLY: we would want to load the Index into memory.
        * IMPORTANT: we have to save the offset of the data section -> that is typically saved near the top of the file.

        RandomAccessFile class:
        * When we want to move to a specific offset in file
            -> we use the RandomAccessFile seek() method to move the File Pointer.
        * We will not always have to do this.
    ...
    private void writingToRandomAccessFile() {
        try (RandomAccessFile raf = new RandomAccessFile(LOCATIONS_RAF_PATH, "rwd")) { // 2. argument -> indicates that we want to open the file for reading and writing and also that we want reads occur synchronously. It can be open read only and can also not specify that updates to the file have to occur synchronously. But if we do that and multiple threads it can access the file, you would actually be responsible for synchronizing the code yourself -> GOOD PRACTISE: to have the RandomAccessFile class handle the synchronization when it does matter.
            // 1.
            raf.writeInt(locations.size());

            // 2.
            int indexSize = locations.size() * 3 * Integer.BYTES; // Each Index record will contain 4 integers: Location ID, the offset for the location and size (length) of the location record
            int recordsOffset = (int) (raf.getFilePointer() + Integer.BYTES + indexSize); // Casting needed -> .getFilePointer returns long value -> (currentPos + bytes for writing recordOffset value + indexSize)
            ...
            /*
                ...
                We could write then object and then write the index record for it and so on but that would actually involve a lot of jumping back and forth in the file.
                ...
                Because we want to jump back to the file when we are finished writing the objects, we will save the current position of the File Pointer
    ...
    // Location Random Access File Stream will have to remain open while the application runs -> so the close() stream method have to be provided in wrapper class

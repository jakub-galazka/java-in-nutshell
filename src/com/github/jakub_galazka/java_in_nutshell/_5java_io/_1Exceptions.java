...
/*
    ...
    * GOOD PRACTISE: when catching an exception specify which subclass of exception to catch.
    ...
    So every time a new method is called, it is actually placed onto the stack and when a method returns it is removed from the stack.
    ...
    The exception is said to propagate up the Call Stack, and it keeps getting passed to the calling methods until there is one that actually can handle the exception.
...
public class _1Exceptions {
    ...
    /*
        ...
        * LBYL (look before you leap) -> test that an object is not null before use it (more common in Java).
        * EAFP (easy to ask for forgiveness than permission) -> go ahead and perform the operation then respond to an exception if something goes wrong.
    */
    ...
    /*
        ...
        * Checked (cannot be ignored)
            - These are the exceptions that are checked at compile time -> Handling: catch || throw them back up the Call Stack
        * Unchecked (can be ignored)
            - These are the exceptions that are not checked at compile time -> GOOD PRACTISE: to handle them

        HINT: SIB cannot throw Checked Exceptions because SIB is executed when the class is loaded
    ...
    */
    ...
    private void throwingBackCheckedException() throws IOException {    // Throw back Checked Exception to handle it in any parent method
    ...
    private static int getInt() {
        ...
        while (true) {
            try {
                return scanner.nextInt(); // Using Ctrl+D - ASCI eot (end of transmission character) -> that abruptly terminates any input -> ERROR: NoSuchElementException

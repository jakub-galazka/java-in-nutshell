...
public class Class extends SuperClass {
    ...
    public Class(int superClassField, int subClassField) {
    ...
    /*
        Polymorphism - the mechanism in OOP that allows actions to act differently based on the actual object that the action is being performed on

        SuperClass superClass = new SuperClass(0);
        superClass.method();                            // OUTPUT: SuperClass -> superClassMethode()
        SuperClass subClass = new Class(0, 0);
        subClass.method();                              // OUTPUT: SuperClass -> methode()
                                                                   Class -> methode() (overridden)
    */
    @Override
    public void method() {
        super.method(); // OPTIONAL: super class method call
        System.out.println("Class -> methode() (overridden)");
    }
}
